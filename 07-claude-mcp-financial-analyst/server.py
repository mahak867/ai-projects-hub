...
if not _api_key:
    print(
        ...
        "Fix: Copy .env.example to .env and add your key from https://console.anthropic.com",
        file=sys.stderr,
    )
    sys.exit(1)
...
async def list_tools() -> list[types.Tool]:
    """Return all available financial analysis tools."""
    return [
        types.Tool(
            name="get_stock_data",
...
async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
    ...
    try:
        if name == "get_stock_data":
            return await _get_stock_data(arguments)
        elif name == "screen_stocks":
            return await _screen_stocks(arguments)
        elif name == "calculate_returns":
            return await _calculate_returns(arguments)
...
async def _get_stock_data(arguments: dict) -> list[types.TextContent]:
    ...
    if hist.empty:
        return [types.TextContent(type="text", text=f"No data found for {symbol}. Check the symbol format (NSE: RELIANCE.NS, BSE: RELIANCE.BO).")]
...
async def _screen_stocks(arguments: dict) -> list[types.TextContent]:
    """Screen a list of stocks by fundamental criteria."""
    symbols: list[str] = arguments["symbols"]
    min_roe: float = arguments.get("min_roe", 0)
    max_pe: float = arguments.get("max_pe", float("inf"))
    min_cap: float = arguments.get("min_market_cap_cr", 0)
...
async def _calculate_returns(arguments: dict) -> list[types.TextContent]:
    """Calculate portfolio and individual stock returns with risk metrics."""
    symbols: list[str] = arguments["symbols"]
    weights: list[float] = arguments.get("weights", [1 / len(symbols)] * len(symbols))
    period: str = arguments.get("period", "1y")
...
async def _get_sector_performance(arguments: dict) -> list[types.TextContent]:
    """Get performance of major NSE sector indices."""
    ...
    for sector_name, sym in sectors.items():
        try:
            hist = yf.Ticker(sym).history(period=period)
            if not hist.empty:
                ret = ((hist['Close'].iloc[-1] - hist['Close'].iloc[0]) / hist['Close'].iloc[0] * 100)
                results[sector_name] = round(float(ret), 2)
        except Exception:
            pass
